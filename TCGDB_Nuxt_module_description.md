...
1. 侧边栏是否直接把 `Customers` 显示为 `Brands`？
2. 品牌详情路由是否接受 `/customers/[id]`，还是必须改成 `/brands/[id]`？
3. News/社媒 MVP 是先用静态示例消息，还是已经有可导入数据？
4. Home 的 TCGplayer 价格信息是否进入第一版，还是只先展示产品数、系列数、活跃产品、活跃系列？
5. Settings 是否需要在第一版加入品牌关键词和数据源配置，还是先保留模板设置页不动？
